...
from mistralai import Mistral # Nouvelle version du SDK
...
# Nouveau client Mistral (plus simple)
...
async def deepseek_refine(llama_data: dict, mistral_data: dict):
    ...
    prompt = f"""
    Analyse croisée de deux experts vision :
    Expert 1 : {json.dumps(llama_data)}
    Expert 2 : {json.dumps(mistral_data)}
    
    Ta mission : Générer une requête de recherche GOOGLE SHOPPING ultra-précise.
    Inclus la marque, le matériau et les cotes techniques.
    Réponds UNIQUEMENT via JSON : {{"search_query": "terme précis", "check": "une question si doute technique"}}
    """
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            return json.dumps({"search_query": llama_data.get('search', 'pièce technique industrielle')})
...
async def search_perplexity(query: str):
    ...
    data = {
        "model": "sonar",
        "messages": [
            {"role": "system", "content": "Trouve 3 fiches produits directes. INTERDIT : catégories ou pages de recherche. Format: Nom - Prix - [Cliquer ici](URL)."},
...
async def identify(image: UploadFile = File(...), context: str = Form("")):
    try:
        ...
        # 1. Analyse Groq (Llama 4 Scout) - On utilise asyncio.to_thread pour ne pas bloquer
        llama_task = asyncio.to_thread(groq_client.chat.completions.create,
            model="meta-llama/llama-4-scout-17b-16e-instruct",
            messages=[{"role": "user", "content": [{"type": "text", "text": "JSON: mat, std, search."}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{img_b64}"}}]}],
            response_format={"type": "json_object"}
        )

        # 2. Analyse Mistral Pixtral (Correction de l'appel API)
        # On utilise le nouveau client asynchrone pour Mistral
        mistral_res = await mistral_client.chat.complete_async(
            model="pixtral-12b-2409",
            messages=[{
                "role": "user",
                "content": [
                    {"type": "text", "text": "Identifie précisément cette pièce technique."},
                    {"type": "image_url", "image_url": f"data:image/jpeg;base64,{img_b64}"}
                ]
            }]
        )

        llama_completion = await llama_task
        l_data = json.loads(llama_completion.choices[0].message.content)
        ...
        <div class="results animate-in">
            <div class="res-card mat"><strong>🧪 Matière</strong><p>{l_data.get('mat')}</p></div>
            <div class="res-card std"><strong>📐 Technique</strong><p>{l_data.get('std')}</p></div>
            <div class="res-card shop"><strong>🛒 Sourcing de Précision</strong>
                <p style="font-size:0.7rem; color:#64748b">Requête : {ds_json.get('search_query')}</p>
                <div class="links-list">{final_links}</div>
            </div>
            {f'<div class="box-warn">⚠️ {ds_json["check"]}</div>' if ds_json.get('check') else ''}
...
def home():
    ...
    <head>
        ...
        <style>
            ...
            textarea { width: 100%; padding: 15px; border: 1px solid #ddd; border-radius: 12px; box-sizing: border-box; min-height: 70px; }
            ...
            .box-warn { background: #fff7ed; border: 1px solid #ffedd5; padding: 12px; border-radius: 10px; color: #9a3412; font-size: 0.9rem; margin-top:10px; font-weight: bold; }
    ...
    <body>
        <div class="container">
            ...
            <img id="preview">
            <div class="input-box">
                <textarea id="ctx" placeholder="Décrivez le problème ou le lieu..."></textarea>
                <button class="mic-btn" onclick="dictate()">🎙️</button>
            </div>
            <button id="go" class="btn btn-run" onclick="run()">LANCER L'ANALYSE</button>
            <div id="loader">⚙️ Triple Vérification en cours...</div>
            <div id="res"></div>
        </div>
        <script>
            let img;
            ...
            async function run() {
                ...
                } catch (e) { alert("Erreur"); } finally { document.getElementById('loader').style.display="none"; }
            }
        </script>
    </body>
    </html>
